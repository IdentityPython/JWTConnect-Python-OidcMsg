...
from oidcmsg.server.cookie_handler import compute_session_state
...
class TestCookieSign(object):
    ...
    def test_make_cookie_content(self):
        _cookie_info = self.cookie_handler.make_cookie_content("oidcmsg.server", "value", "sso")
        assert _cookie_info
    ...
    def test_make_cookie_content_max_age(self):
        _cookie_info = self.cookie_handler.make_cookie_content(
            "oidcmsg.server", "value", "sso", max_age=3600
        )
        assert _cookie_info
        assert set(_cookie_info.keys()) == {
    ...
    def test_read_cookie_info(self):
        ...
        _info = self.cookie_handler.parse_cookie("oidcmsg.server", returned)
        assert len(_info) == 1
    ...
    def test_mult_cookie(self):
        _cookie = [
            self.cookie_handler.make_cookie_content("oidcmsg.server", "value", "sso"),
            self.cookie_handler.make_cookie_content("oidcmsg.server", "session_state", "session"),
        ]
        assert len(_cookie) == 2
        _c_info = self.cookie_handler.parse_cookie("oidcmsg.server", _cookie)
